...
<template>
  <Transition>
    <div
      ...
      ref="$window"
      class="window fixed min-w-[100px] shadow-2xl rounded-xl z-50"
    >
      <!-- header -->
      <div
        @mousedown="handleMouseDown_move"
      ...
      >
        ...
        <div class="flex justify-end gap-2">
          <button
            @click.stop="zoom"
            class="rounded-full p-2 bg-yellow-300 hover:bg-yellow-400 active:bg-yellow-500"
          />
          <button
            @click.stop="close"
            class="rounded-full p-2 bg-red-700 hover:bg-red-800 active:bg-red-900"
          />
        </div>
      </div>
      <!-- body -->
      <div
        ref="$body"
        class="body w-full bg-purple-200 overflow-scroll"
        :class="props.class"
      >
        <slot></slot>
      </div>
      <!-- footer -->
      <div
        class="w-full h-[18px] rounded-b-xl bg-purple-400 flex justify-end overflow-hidden"
      >
        <div
          @mousedown="handleMouseDown_resize"
          class="p-2 cursor-se-resize"
        ></div>
      </div>
    </div>
  </Transition>
...
.window {
  ...
  transform: translate(var(--movedX), var(--movedY));
...
.v-enter-from,
.v-leave-to {
  transform: translate(var(--movedX), calc(var(--movedY) + -10px));
  opacity: 0;
}
.v-enter-to,
.v-leave-from {
