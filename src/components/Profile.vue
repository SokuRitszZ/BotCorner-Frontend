  ...
  <div class="relative w-96 h-full flex items-center">
    <Transition name="profile">
      ...
      <div key="not" v-if="userStore.status === 'not logged in'" class="absolute right-0">
        <div class="flex justify-center items-center gap-2">
          <input placeholder="用户名" v-model="username" name="username" class="px-2 rounded-md w-40" type="text">
          <input placeholder="密码" v-model="password" name="password" class="px-2 rounded-md w-40" type="password">
          <soku-button @click="login" type="normal" class="px-2 py-1 font-bold whitespace-nowrap">
            登录
          </soku-button>
          <soku-button @click="toRegister" type="success" class="px-2 py-1 font-bold whitespace-nowrap">
            注册
          </soku-button>
          <soku-window ref="$window_register" title="注册">
            <div class="w-full h-full p-3 m-auto flex flex-col gap-3">
              ...
              <input v-model="register_info['username']" name="username" type="text" class="w-full col-span-2 rounded-md px-2 outline-purple-700">
              <label class="" for="password">密码</label>
              <input v-model="register_info['password']" name="password" type="password" class="w-full col-span-2 rounded-md px-2 outline-purple-700">
              <label class="" for="confirmed_password">确认密码</label>
              <input v-model="register_info['confirmed_password']" name="confirmed_password" type="password" class="w-full col-span-2 rounded-md px-2 outline-purple-700">
              <soku-button @click="register" class="px-3 py-1 m-auto">注册</soku-button>
            </div>
          </soku-window>
        </div>
      </div>
    ...
    <Transition name="user-menu">
      <div
        ...
        <p @click="toUpdateAvatar" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200" role="menuitem" tabindex="-1" id="user-menu-item-0">更换头像</p>
        <p @click="logout" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200" role="menuitem" tabindex="-1" id="user-menu-item-1">登出</p>
      </div>
    </Transition>
  </div>
</template>
