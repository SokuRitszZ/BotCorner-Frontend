...
<template>
  ...
  >
    <div ref="$slider" class="flex w-fit slider">
      <div
        :class="[`w-[${props.width}px]`, `h-[${props.height}px]`, 'p-3']"
        v-for="game in games"
        :key="game.id"
      >
        <div
          class="board cursor-pointer w-full h-full rounded-xl p-10"
          @click="router.push(`/game/${game.title}`)"
        >
          <h1 class="title text-purple-500 text-5xl text-center font-semibold">
            {{ game.name }}
          </h1>
          <hr class="hr my-5" />
