...
const setButtonPosition = (newV: number) => {
  ...
  $button.value.style.setProperty('--moved', `${newV / props.max * props.width}px`);
}
...
<template>
  <div :class="props.class" class="select-none w-fit m-auto pr-4 rounded-full relative flex items-center gap-1">
    ...
    <div ref="$progress" class="w-fit flex items-center">
      ...
      <div @mousedown="handleMousedown" id="button" ref="$button" class="cursor-pointer w-[14px] h-[14px] bg-purple-700 rounded-full absolute" />
    </div>
  </div>
</template>
