...
class Selector extends GameObject {
  ...
  protected onStart(): void {
    this.addUpdater("render selector", new Updater(() => {
      const L = this.root.L;
      const { x, y } = this.p;
      if (x === -1) return ;
      G.StrokePoly({
    ...
    this.addUpdater("move selector", new Updater(() => {
      if (!this.tp) return ;
  ...
  public setP(p: IPosition | null) {
    ...
    if (this.p.x === -1) this.p = p; 
      ...
      if (this.tp === null && (this.p.x !== p.x || this.p.y !== p.y) || (this.tp !== null) && (this.tp.x !== p.x || this.tp.y !== p.y)) {
        this.tp = p;
