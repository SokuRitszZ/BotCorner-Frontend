...
import Select, { IEntry } from '@/components/Select.vue';
  ...
  <div class="grid grid-cols-5 min-w-[1600px]">
    ...
    <div class="col-span-2 h-screen border-r-[1px] border-gray-300 overflow-scroll">
      <div class="flex justify-between items-center pt-24 pb-3 px-10">
        ...
        <div>
          <Select class="border-[1px] p-1 px-3" :list="comparersList" v-model="comparer" />
        </div>
      </div>
      ...
      <TransitionGroup name="bots">
        <div @click="selectBot(bot)" :key="bot.id" v-for="bot in bots"
          :class="{'bg-purple-600 text-white': selectedBot === bot}" class="select-none bot transition border-b-[1px] px-10 py-4 bg-white hover:bg-purple-600 hover:text-white active:bg-purple-800 cursor-pointer">
          <div class="flex justify-between">
            <div :class="{'text-purple-600': selectedBot !== bot}" class="title font-semibold flex justify-between items-center gap-2">
              <Icon type="bot" :size="24" />
              {{ bot.title }}
            </div>
            <div :class="{'text-gray-400': selectedBot !== bot}" class="font-color rounded-full text-sm h-fit p-1 font-bold flex justify-between gap-3 items-center">
              {{ toWord(cacheStore.getLang(bot.langId)) }}
              <Icon v-if="bot.visible" @click.stop="toggle(bot.id, false)" type="eye" :size="24" />
              <Icon v-else @click.stop="toggle(bot.id, true)" type="eye-slash" :size="24" />
            </div>
          </div>
          <div class="flex justify-between">
            ...
            <div class="flex gap-2">
              <span :class="{'text-gray-400': selectedBot !== bot}" class="font-color">
