<template>
  <div :class="`pt-24 px-16 h-screen overflow-scroll`">
    <h1 class="text-4xl font-bold relative items-center">
      ...
      <div class="block absolute right-0 top-0 text-xl font-normal">
        <button
          ...
          @click="submit"
          class="bg-green-700 disabled:bg-green-500 hover:bg-green-800 active:bg-green-900 text-white px-3 py-2 rounded-lg"
        >
          提交
        </button>
      </div>
    ...
    <hr class="p-3 mt-5" />
    <div>
      <p class="font-semibold text-xl">名称</p>
      <input
        v-model="title"
    ...
    <div class="pt-5">
      <p class="font-semibold text-xl relative mb-10">代码</p>
      <textarea
        ref="$editor"
        class="h-screen mt-2 w-full border-[1px] rounded-xl p-3"
      />
    </div>
  </div>
</template>
