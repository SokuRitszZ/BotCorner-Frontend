...
function toWatch(uuid: string) {
  const playing: IPlaying | undefined = props.list_play.find(
    (p) => p.uuid === uuid
  );
  if (!playing) return;
...
<template>
  ...
  <div :class="class">
    ...
    <TransitionGroup v-else>
      <div
        class="bg-purple-700 p-3 w-full rounded-xl flex justify-between items-center"
        :class="{ 'mt-3': index > 0 }"
        :key="playing.uuid"
      ...
      >
        ...
        <Icon
          @click="toWatch(playing.uuid)"
          class="hover:text-purple-800 active:text-purple-900"
          type="play"
          :size="24"
        />
      </div>
