<template>
  <div class="h-2/5 w-full flex flex-col-reverse">
    <div class="w-full flex justify-between items-center">
      <img class="h-24 rounded-full border-black" :class="{'border-8': gameStore.users.findIndex(user => user.id === userStore.id) === 0}" :src="gameStore.users[0].avatar" alt="avatar">
  ...
  <div class="font-bold text-4xl h-1/5 flex justify-center gap-10 items-center">
    ...
    <div class="flex items-center">
      <div v-if="turn === 0" class="relative">
        <div class=" bg-white rounded-full relative w-16 top-1 h-16 drop-shadow-2xl"></div>
        <div class=" bg-black rounded-full absolute top-0 w-16 h-16"></div>
      </div>
      <div v-else-if="turn === 1" class="relative">
        <div class=" bg-black rounded-full relative w-16 top-1 h-16 drop-shadow-2xl shadow-white"></div>
  ...
  <div class="h-2/5 w-full flex flex-col">
    <div class="w-full flex justify-between items-center">
      <img class="h-24 rounded-full border-white" :class="{'border-4': gameStore.users.findIndex(user => user.id === userStore.id) === 1}" :src="gameStore.users[1].avatar" alt="avatar">
      <div class="relative w-full h-full flex justify-center items-center">
