...
<template>
  <div
    :class="[props.class, 'bg-purple-500 rounded-3xl shadow-2xl p-7 w-full']"
  >
    <ChatRoom ref="$chatroom" @send="send" />
    <div class="w-full mt-3">
      <h1 class="text-center text-3xl flex justify-between items-center">
        <button
          :disabled="status !== 'to start'"
          @click="mode = 'single'"
          :class="{
            'bg-purple-700 text-white': mode === 'single',
            'text-purple-700': mode !== 'single',
          }"
        ...
        <button
          ...
          class="text-white text-xl bg-green-700 py-1 px-3 rounded-lg hover:bg-green-800 active:bg-green-900"
        >
          开始
        </button>
      </h1>
      <input
        :disabled="disabled_single"
        placeholder="Bot的编号，不填代表手动"
        v-model="single_botId[0]"
        type="number"
    ...
    <div class="w-full pt-10">
      <h1 class="text-3xl text-center flex justify-between items-center">
        <button
          ...
          @click="mode = 'multi'"
          :class="{
            'bg-purple-700 text-white': mode === 'multi',
            'text-purple-700': mode !== 'multi',
          }"
          class="cursor-pointer p-2 rounded-xl transition border-2 border-purple-700"
        >
          多人模式
        ...
        <button
          :disabled="disabled_exit"
          @click="exitMatching"
          class="text-white text-xl bg-red-700 py-1 px-3 rounded-lg hover:bg-red-800 active:bg-red-900"
        >
          退出
        </button>
        <button
    ...
    <div class="w-full pt-3 relative">
      <Transition>
        <!-- matching -->
        <div
          v-if="match_status === 'matching'"
          class="w-full flex justify-center items-center gap-5 absolute left-0"
        >
          <img
            :src="userStore.avatar"
            class="rotate rounded-full w-16 h-16"
            alt="avatar"
          />
          <div class="text-2xl">寻找对手中......</div>
        </div>
        <!-- matched -->
        <div
          v-else-if="match_status === 'matched'"
          class="w-full absolute left-0"
        >
          <div class="w-full flex justify-between items-center">
            <ImageHoverDetail
              @click="ok(0)"
              :src="multi_userData[0].avatar"
              :class="`w-16 h-16 rounded-full border-4 cursor-pointer transition ${
                multi_isOk[0] ? 'border-purple-800' : 'border-red-700'
              }`"
            >
              <h1 class="whitespace-nowrap">
                {{ multi_userData[0].username }}#{{
                  leftpad(8, multi_userData[0].id)
                }}
              </h1>
            </ImageHoverDetail>
            <div class="text-white">点击头像准备</div>
            <ImageHoverDetail
              @click="ok(1)"
              :src="multi_userData[1].avatar"
              :class="`w-16 h-16 rounded-full border-4 cursor-pointer transition ${
                multi_isOk[1] ? 'border-purple-800' : 'border-blue-700'
              }`"
            >
              <h1 class="whitespace-nowrap">
                {{ multi_userData[1].username }}#{{
                  leftpad(8, multi_userData[1].id)
                }}
              </h1>
            </ImageHoverDetail>
          </div>
        ...
        <!-- waiting -->
        <div
          v-else-if="match_status === 'waiting'"
          class="w-full flex justify-center items-center text-xl text-gray-50"
        >
          等待Bot编译完成......
        </div>
      </Transition>
